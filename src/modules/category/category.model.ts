...
const categorySchema = new Schema<ICategoryDocument>(
  {
    ...
    // Category-level tax/fulfillment defaults inherited by every product in the
    // category unless the product sets its own (product wins). Resolved at response
    // time by flattenProductCatalog, like `gst`, so they need not be repeated per product.
